// ============================================================
// CloudForge — Progress Persistence Hook
...
const STORAGE_KEY = "cloudforge_progress";
const PREFS_KEY = "cloudforge_progress";
...
function createDefaultProgress(): CloudForgeProgress {
...
function migrateProgress(data: Record<string, unknown>): CloudForgeProgress {
  ...
  return data as unknown as CloudForgeProgress;
}

function loadProgress(): CloudForgeProgress {
  try {
    ...
    if (raw) {
      ...
      if (!validated) {
        console.warn("[CloudForge] Progress data failed validation — resetting.");
...
function saveProgress(progress: CloudForgeProgress) {
  progress.updatedAt = new Date().toISOString();
  try {
    localStorage.setItem(STORAGE_KEY, JSON.stringify(progress));
  } catch (error) {
    // QuotaExceededError: localStorage is full (5-10 MiB limit in WebView)
    console.error("[CloudForge] Failed to save progress:", error);
    ...
    try {
      ...
      console.error(
        "[CloudForge] Even minimal save failed. Progress may be lost."
      );
    }
  }
}
 ...
 */
function mirrorToPreferences(progress: CloudForgeProgress) {
  Preferences.set({
    key: PREFS_KEY,
    value: JSON.stringify(progress),
  }).catch((err) => {
    console.warn("[CloudForge] Failed to mirror progress to Preferences:", err);
  });
}
 ...
 */
async function recoverFromPreferences(): Promise<CloudForgeProgress | null> {
  try {
    ...
    if (value) {
      ...
      if (validated) {
        const migrated = migrateProgress(validated);
        console.info("[CloudForge] Recovered progress from Preferences backup.");
        return migrated;
      }
    }
  } catch {
...
export function useProgress(userId?: string | null) {
  const [progress, setProgress] = useState<CloudForgeProgress>(loadProgress);
      ...
      recoverFromPreferences().then((recovered) => {
        if (recovered) {
          ...
          if (userId && recovered.userId && recovered.userId !== userId) {
            console.warn(
              `[CloudForge] Recovered progress belongs to UID ${recovered.userId}, active UID is ${userId}. Discarding stale backup.`
            );
            return;
          }
...
export type { CloudForgeProgress, LabProgress, ScenarioResult };
