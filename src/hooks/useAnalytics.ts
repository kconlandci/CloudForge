// ============================================================
// CloudForge — Lightweight Analytics Hook
// Tracks user events in localStorage for local usage insights.
// ============================================================

import { getLocalDateString } from "../utils/localDate";

const ANALYTICS_KEY = "cloudforge_analytics";
...
function loadEvents(): AnalyticsEvent[] {
  try {
    ...
    if (!Array.isArray(parsed)) {
      console.warn("[CloudForge Analytics] Corrupted data (non-array) — resetting.");
...
function pushEvent(event: AnalyticsEvent) {
  try {
    ...
    console.error("[CloudForge Analytics] Failed to save event:", err);
  }
}
