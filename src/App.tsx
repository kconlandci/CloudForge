...
export default function App() {
  const [isRevenueCatReady, setIsRevenueCatReady] = useState(
    !Capacitor.isNativePlatform() // On web, treat as immediately ready (no RevenueCat)
  );
  ...
  useEffect(() => {
    if (Capacitor.isNativePlatform()) {
      Purchases.configure({ apiKey: REVENUECAT_API_KEY })
        ...
        .catch((err) => {
          console.error("[CloudForge] RevenueCat init failed:", err);
          setIsRevenueCatReady(true); // Unblock the app even on failure
        });
    }
  }, []);

  // Analytics: track app open, resume, and session duration
  useEffect(() => {
    trackAppOpened();
