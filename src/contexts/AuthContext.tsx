...
const UID_STORAGE_KEY = "cloudforge_firebase_uid";
      ...
      if (user) {
        try {
          ...
          if (storedUid && storedUid !== currentUid) {
            console.warn(
              `[CloudForge] UID changed: stored=${storedUid}, current=${currentUid}. ` +
          ...
          console.error("[CloudForge] UID persistence failed:", error);
        } finally {
          setIsAuthReady(true);
        }
      } else {
        ...
        try {
          ...
          console.error("[CloudForge] Anonymous sign-in failed:", error);
          setUid(null);
          setIsAuthReady(true);
        }
      }
