...
const COLOR_MAP: Record<string, { selected: string; base: string }> = {
  ...
  orange: { selected: "bg-violet-500/20 border-violet-500 text-violet-300", base: "bg-slate-800 border-slate-700 text-slate-300 hover:border-slate-500" },
    ...
    <div className="space-y-4">
      ...
      {phase === "active" && (
        <button
          ...
          className="w-full py-3 rounded-xl bg-violet-500 hover:bg-violet-600 disabled:bg-slate-700 disabled:text-slate-500 text-white font-semibold transition-colors"
        >
          Submit Answer
        </button>
      )}
